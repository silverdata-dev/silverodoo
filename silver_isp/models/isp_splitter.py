...
class IspSplitter(models.Model):
    ...
    olt_card_port_id = fields.Many2one('isp.olt.card.port', string='Puerto Tarjeta OLT', required=True, ondelete='cascade')
    ...
    def create(self, vals):
        if vals.get('olt_card_port_id'):
            port = self.env['isp.olt.card.port'].browse(vals['olt_card_port_id'])
            if port.exists():
        ...
        return super(IspSplitter, self).create(vals)

    def write(self, vals):
        # Determine the new name if the parent port or the type changes
        if 'olt_card_port_id' in vals or 'type_splitter' in vals:
            for record in self:
                port = self.env['isp.olt.card.port'].browse(vals.get('olt_card_port_id', record.olt_card_port_id.id))
        ...
        return super(IspSplitter, self).write(vals)
