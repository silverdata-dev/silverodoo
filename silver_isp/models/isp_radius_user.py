...
class IspRadiusUser(models.Model):
    _name = 'isp.radius.user'
    _description = 'RADIUS User'

    radius_id = fields.Many2one('isp.radius', string='RADIUS Server', required=True, ondelete='cascade')
    ...
    def create(self, vals):
        record = super(IspRadiusUser, self).create(vals)
       # record._sync_to_mikrotik('add')
        return record

    def write(self, vals):
        res = super(IspRadiusUser, self).write(vals)
        #self._sync_to_mikrotik('set')
        return res

    def unlink(self):
        #self._sync_to_mikrotik('remove')
        return super(IspRadiusUser, self).unlink()
