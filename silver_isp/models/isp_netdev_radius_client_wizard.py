...
class IspNetdevRadiusClientWizard(models.TransientModel):
    _name = 'isp.netdev.radius.client.wizard'
    _description = 'Manage MikroTik Radius Client Configuration'

    netdev_id = fields.Many2one('isp.netdev', string='Network Device', required=True)
    ...
    radius_client_services = fields.Many2many('isp.radius.service', string='Radius Services', default=lambda self: self.env['isp.radius.service'].search([('name', 'in', ['ppp', 'hotspot'])]))
